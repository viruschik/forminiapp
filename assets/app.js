...
(function bootstrap(){
  ...
  function renderForm(){const w=el.formArea;if(!w)return;w.classList.remove('placeholder');
    ...
    else{w.classList.add('placeholder');w.innerHTML=`<p>${tx().placeholder}</p>`;try{tg?.MainButton?.hide();tg?.MainButton?.offClick?.(payNow);}catch{} hideFakeMainButton();return;}
    attachFormHandlers();
  }

  function premiumFormHTML(){const t=tx();const me=(tg?.initDataUnsafe?.user?.username)?'@'+tg.initDataUnsafe.user.username:(state.lang==='ru'?'(в Telegram)':'(in Telegram)');return`
    <div class="form-grid">
  ...
  function starsBuyFormHTML(){const t=tx();const me=(tg?.initDataUnsafe?.user?.username)?'@'+tg.initDataUnsafe.user.username:(state.lang==='ru'?'(в Telegram)':'(in Telegram)');return`
    <div class="form-grid">
      <div class="row cols-2">
        <div class="radio-group">
  ...
  function starsSellFormHTML(){const t=tx();return`
    <div class="form-grid">
      <div class="row cols-2">
        <div class="input">
  ...
  function attachFormHandlers(){
    const goodUsername=(u)=>/^@?[a-zA-Z0-9_]{5,}$/.test((u||'').trim());

    if(state.action==='premium_buy'){
      const radios=document.querySelectorAll('input[name="to"]');
      const username=$('username'); const months=$('months'); const currency=$('currency');
      const priceLine=$('priceLine'); const userErr=$('userErr'); const yourHint=$('yourNameHint');
      ...
      async function validateUserFlow(){
        ...
        userErr.hidden=true;
        const chk=await apiCheckUser(u); if(!chk.exists){userErr.textContent=tx().errUserNotFound;userErr.hidden=false;return false;}
        const prem=await apiCheckPremium(u); if(prem.active){userErr.textContent=tx().errPremiumActive;userErr.hidden=false;return false;}
      ...
      radios.forEach(r=>r.addEventListener('change',async()=>{
        ...
        username.disabled=(val!=='other'); if(val==='self'){userErr.hidden=true; yourHint&& (yourHint.style.display='block');}
        else {yourHint&&(yourHint.style.display='none'); await validateUserFlow();}
        updateMainButton();
      }));
      username?.addEventListener('input',async()=>{await validateUserFlow();updateMainButton();});

      function recalcPrice(){const m=state.form.months; const cur=state.form.currency; const t=tx();
        if(m&&cur&&PRICES.premium[m]?.[cur]!=null){state.form.amount=PRICES.premium[m][cur];priceLine.textContent=`${t.price}: ${state.form.amount} ${String(cur).replace('_TON','')}`;}
    ...
    if(state.action==='stars_buy'){
      const radios=document.querySelectorAll('input[name="to"]');
      const username=$('username'); const amount=$('starsAmount'); const currency=$('currency');
      const priceLine=$('priceLine'); const userErr=$('userErr'); const yourHint=$('yourNameHint');
      ...
      radios.forEach(r=>r.addEventListener('change',async()=>{
        const val=document.querySelector('input[name="to"]:checked').value; state.form.to=val;
        username.disabled=(val!=='other'); yourHint&&(yourHint.style.display=(val==='self')?'block':'none');
        await validateRecipient(); updateMainButton();
      }));
      username?.addEventListener('input',async()=>{await validateRecipient();updateMainButton();});

      function recalc(){const stars=parseInt(amount?.value??'0',10)||0; state.form.stars=stars; const cur=state.form.currency; const t=tx();
        ...
        else{const total=+(stars*rate).toFixed(6); state.form.amount=total; priceLine.textContent=`${t.totalPay}: ${total} ${String(cur).replace('_TON','')}`;}
        updateMainButton();}
    ...
    if(state.action==='stars_sell'){
      const amount=$('starsAmount'); const currency=$('currency'); const priceLine=$('priceLine');
      const addrRow=$('addressRow'); const addrInp=$('payoutAddress'); const sellErr=$('sellErr');

      function validate(){const t=tx(); const stars=parseInt(amount?.value??'0',10)||0; const cur=state.form.currency; const addr=(addrInp?.value||'').trim();
        let ok=true; if(stars<50){sellErr.textContent=t.errMinStars;sellErr.hidden=false;ok=false;} else sellErr.hidden=true;
        if(!cur) ok=false; if(cur&&addrRow&&!addrRow.hidden){ if(!addr) ok=false; } return ok; }

      function recalc(){const t=tx(); const stars=parseInt(amount?.value??'0',10)||0; state.form.stars=stars; const cur=state.form.currency;
        ...
        else{const total=+(stars*rate).toFixed(6); state.form.amount=total; priceLine.textContent=`${t.youGet}: ${total} ${String(cur).replace('_TON','')}`;}
        updateMainButton();}
      amount?.addEventListener('input',()=>{validate();recalc();});
      currency?.addEventListener('change',()=>{state.form.currency=currency.value; if(addrRow){addrRow.hidden=!state.form.currency; if(addrInp){addrInp.placeholder=(state.form.currency==='TON')?'ton:// or 0:...':(state.lang==='ru'?'Адрес USDT (TON)':'USDT (TON) address');}} validate(); recalc();});
  ...
  function updateMainButton(){const t=tx(); let ok=false,label=t.btnPay;
    if(state.action==='premium_buy'){const to=state.form.to||'self'; const usernameOk=(to==='self')||(/^@?[a-zA-Z0-9_]{5,}$/.test(document.getElementById('username')?.value||''));
      ...
      if(ok) label=(state.lang==='ru')?`Оплатить ${state.form.amount} ${String(state.form.currency).replace('_TON','')}`:`Pay ${state.form.amount} ${String(state.form.currency).replace('_TON','')}`;}
    if(state.action==='stars_buy'){const to=document.querySelector('input[name="to"]:checked')?.value||'self'; const userOk=(to==='self')||(/^@?[a-zA-Z0-9_]{5,}$/.test(document.getElementById('username')?.value||''));
      ok=!!(userOk&&state.form.stars>0&&state.form.currency&&state.form.amount!=null);
      if(ok) label=(state.lang==='ru')?`Оплатить ${state.form.amount} ${String(state.form.currency).replace('_TON','')}`:`Pay ${state.form.amount} ${String(state.form.currency).replace('_TON','')}`;}
    if(state.action==='stars_sell'){const isValid=(state.form.stars>=50)&&!!state.form.currency&&(!document.getElementById('addressRow')?.hidden?!!(state.form.address&&state.form.address.length>5):true);
      ok=isValid&&state.form.amount!=null; if(ok) label=(t.btnTransfer||'Передать {n} звёзд').replace('{n}',state.form.stars);}
    try{ if(tg){ if(ok && el.sidebar?.getAttribute('aria-hidden')!=='false'){ tg.MainButton.offClick?.(payNow); tg.MainButton.setText(label); tg.MainButton.show(); tg.MainButton.onClick(payNow); } else { tg.MainButton.hide(); tg.MainButton.offClick?.(payNow); } }
